var svg = d3.select("svg"),
    margin = { top: 20, right: 40, bottom: 120, left: 80 },
...
d3.csv("Air_Quality.csv", function (d) {
    ...
    function onchange() {
        ...
        nestdata = d3.nest().key(function (d) { return d.geo_entity_name }).rollup(function (leaves) {
            return {
                "total": d3.mean(leaves, function (d) { return d.data_valuemessage }),
                "parent": d3.mean(leaves, function (d) { return d.parent_id }),
                "loc": d3.mean(leaves, function (d) { return d.correct_geo_id })
            }
        }).entries(subset)
